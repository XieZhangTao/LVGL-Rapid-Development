...
def find_workspace_root(start_dir):
    """Find the workspace root that contains the LVGL project files."""
    current_dir = Path(start_dir).resolve()
    for candidate_dir in [current_dir, *current_dir.parents]:
        if ((candidate_dir / 'CMakeLists.txt').exists()
                and (candidate_dir / 'lvgl').exists()):
...
class ResourceGenerator:
    ...
    def generate_images(self):
        ...
        image_stats = []
        project_root = find_workspace_root(Path(__file__).parent)
        lv_img_conv_candidates = [
            project_root / 'node_modules' / 'lv_img_conv' / 'lv_img_conv.js',
            Path(__file__).parent.resolve() / 'node_modules' / 'lv_img_conv' / 'lv_img_conv.js'
        ]
        lv_img_conv_path = next(
            (candidate for candidate in lv_img_conv_candidates if candidate.exists()),
            lv_img_conv_candidates[0]
        )
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        font_config = workspace_root / 'font_config.json'
        image_config = workspace_root / 'image_config.json'
