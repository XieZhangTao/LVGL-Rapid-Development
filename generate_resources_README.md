...
`generate_resources.py` 用于根据 JSON 配置自动生成 LVGL 资源文件，并维护统一声明头文件 `lv_resource.h`。
...
```bash
npm install lv_font_conv lv_img_conv ts-node
pip install Pillow
```
...
```bash
python generate_resources.py path/to/font_config.json path/to/image_config.json
```
...
- 默认模式下脚本目录存在 `font_config.json` 或 `image_config.json`
...
```bash
npm install lv_img_conv ts-node
```
