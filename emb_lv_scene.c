...
#define emb_lv_scene_get_current_scene()                        \
    list_entry(tSceneManage.tSceneList.ptNext, lv_scene_t, tSceneList)
#define emb_lv_scene_get_previous_scene()                       \
    list_entry(tSceneManage.tSceneList.ptNext->ptNext, lv_scene_t, tSceneList)
...
lv_scene_t* emb_lv_scene_create(uint8_t u8SceneID, lv_scene_cfg_t *ptCfg)
{
    ...
    assert(ptScene != NULL && ptScreen != NULL);
...
{
    assert(ptCfg != NULL && ptSceneEX != NULL);
...
void emb_lv_scene_append_to_list(lv_scene_t *ptScene)
{
    ...
    emb_list_add_head(&(ptThis->tSceneList), &tSceneManage.tSceneList);
}
...
void emb_lv_scene_remove_from_list(lv_scene_t *ptScene)
{
    ...
    lv_scene_t *ptThis = ptScene;
    emb_list_del(&(ptThis->tSceneList));
}
...
void emb_lv_scene_load_current_scene_with_del_previous(void)
{
    ...
    do{
        if(&(ptPreviousScene->tSceneList) == &tSceneManage.tSceneList) {
            EMB_LOG_WARNING(
                USER,
        ...
        if(ptPreviousScene->ptSceneEX[0]->ptGroup != NULL)
        {
            ...
            assert(ptFocusObj != NULL); // focus object must be valid
...
{
    lv_scene_t *ptScene = emb_lv_scene_get_current_scene();
    assert(ptScene != NULL); /* should never be NULL */
